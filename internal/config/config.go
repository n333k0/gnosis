// Package config loads gnosis's single configuration file
// (gnosis.yaml / ~/.gnosis/config.yaml / embedded default).
package config
...
const (
	projectConfigName = "gnosis.yaml"
	userConfigDir     = ".gnosis"
...
//go:embed defaults/gnosis.yaml
var embeddedConfigYAML []byte

// Config is the parsed gnosis.yaml.
type Config struct {
	...
	// device-code credentials via `gnosis login`). This also applies when OpenAI
	...
	// Phases overrides Gnosis's built-in named prompts by name or adds custom
...
// Output configures how Gnosis renders tool activity during a chat session.
type Output struct {
	// Verbose restores the full tool call/result rendering (complete file
	// contents, command output, etc). Defaults to false: Gnosis shows live activity
...
// so a minimal gnosis.yaml still gets the full experience; set a flag to false to
...
func Load() (*Config, error) {
	...
	cfg, err := parseConfig(embeddedConfigYAML, "embedded:gnosis.yaml")
	if err != nil {
		return nil, err
	}
	cfg.source = "embedded"
...
// NamedPhases returns Gnosis's defaults with configured definitions overlaid.
