...
import (
	...
	"github.com/n333k0/gnosis/internal/approval"
	"github.com/n333k0/gnosis/internal/compact"
	"github.com/n333k0/gnosis/internal/config"
	"github.com/n333k0/gnosis/internal/factory"
	"github.com/n333k0/gnosis/internal/llm"
	"github.com/n333k0/gnosis/internal/logx"
	"github.com/n333k0/gnosis/internal/phase"
	"github.com/n333k0/gnosis/internal/projectctx"
	"github.com/n333k0/gnosis/internal/session"
	"github.com/n333k0/gnosis/internal/skills"
	"github.com/n333k0/gnosis/internal/tools"
	"github.com/n333k0/gnosis/internal/tui"
	"github.com/n333k0/gnosis/internal/workflow"
	"github.com/n333k0/gnosis/internal/workspace"
)
...
const chatSystemPrompt = `You are gnosis, a focused coding agent.
...
statuses based on evidence. Do not mirror every tool call manually; Gnosis attaches
...
func run(args []string, streams stdio) int {
	...
	defer stop()
	logx.Debug("gnosis start", "args", logx.SafeAny(args))

	// `gnosis` with no subcommand defaults to chat — the common case.
	if len(args) == 0 {
	...
	case "resume":
		if len(args) < 2 {
			fmt.Fprintln(streams.err, "usage: gnosis resume <session-id>")
			return 2
		}
	...
	case "-h", "--help", "help":
		...
		return 0
	default:
		logx.Debug("gnosis unknown command", "command", args[0])
...
const usageText = `gnosis — a Go coding agent

USAGE:
  gnosis                Interactive chat mode (default)
  gnosis chat           Interactive chat mode (explicit)
  gnosis run [options] <prompt>
                     Run one headless prompt and exit
  gnosis sessions       List saved chat sessions
  gnosis sessions search <query>
                     Search saved session transcripts
  gnosis doctor         Check local config and environment
  gnosis resume <id>    Resume a saved chat session
  gnosis login          Log in to an OpenAI ChatGPT/Codex subscription (device code)
  gnosis logout         Remove stored subscription credentials
  gnosis version        Show the Gnosis version (also -v, --version)
  gnosis help           Show this help

CONFIG:
  Reads gnosis.yaml (cwd) → ~/.gnosis/config.yaml → embedded defaults.
  ...
  To use a ChatGPT subscription instead of an API key, set in gnosis.yaml:
    provider: openai
    openai_auth: subscription
  then run "gnosis login".

HEADLESS RUN:
  gnosis run --json --timeout 10m "Review this repo without changing files"
  cat prompt.md | gnosis run --json
...
func printVersion(w io.Writer) {
	fmt.Fprintf(w, "gnosis version %s\n", Version)
}
...
func runHeadless(ctx context.Context, args []string, streams stdio) int {
	...
	if err != nil {
		...
		fmt.Fprintln(streams.err, "usage: gnosis run [--json] [--timeout 10m] <prompt>")
		return 2
	}
	if opts.timeout > 0 {
...
func parseHeadlessArgs(args []string, stdin io.Reader) (headlessOptions, string, error) {
	...
	for _, arg := range args {
		if arg == "--permission" || arg == "-permission" ||
			...
			return opts, "", fmt.Errorf("--permission has been removed; run Gnosis inside a sandbox and use tool_approvals for optional interactive confirmations")
		}
	}
	...
	if prompt == "" {
		return opts, "", fmt.Errorf("gnosis run: missing prompt")
	}
