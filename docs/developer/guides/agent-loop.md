...
The agent loop is Gnosis's heartbeat. A user sends a message, the model answers, Gnosis runs any requested tools, then the model continues until it has a final answer.

Think of it like:

1. User asks for work.
2. Model decides what to do.
3. Gnosis runs tools the model asked for.
...
Gnosis builds the assistant message and tool results together before committing them to the transcript. That keeps the conversation valid even when a tool fails.
