# Gnosis Goal Runner - Design Spec

> **Status:** Draft. This describes proposed behavior, not the current product.

This spec describes how Gnosis should implement `/goal`.

`/goal` gives Gnosis a standing objective that survives across turns in the current session.

Gnosis starts with the goal as the first normal user turn.

After each turn, Gnosis checks whether the goal is still active.

If it is active and Gnosis is idle, Gnosis adds a continuation prompt and keeps working.
...
Gnosis Phase 1 should copy the Codex control-plane idea, but not all of Codex's machinery.

Gnosis should store explicit goal state, expose a narrow goal tool, and continue only from a simple idle scheduler.

Gnosis should skip token accounting, hidden internal context, dynamic tool visibility, and a separate evaluator model in Phase 1.
...
### Why Codex is the better base for Gnosis

Codex is not better because it has more features.

It is better for Gnosis because its core abstraction is product state, not a hidden evaluator decision.
...
Gnosis should offset that with a strong continuation prompt, concrete evidence requirements, a blocked threshold, and normal user review.
...
| Gnosis Phase 1 | Agent calls one narrow goal tool, with manager-enforced transitions. | TUI appends a visible continuation prompt only when idle and input is empty. | Optional `goal` field in session JSON. | Simple, reliable, easy to explain, close to current Gnosis architecture. | Completion quality depends on prompt discipline and evidence checks. |
...
Gnosis can add an optional judge or audit pass later if self-reported completion is too weak.

## 3. Product Definitions

| Term | Meaning | Gnosis feature |
|------|---------|-------------|
...
A goal is the durable control-plane state that decides whether Gnosis should continue.
...
When Gnosis eventually adds scheduled work, design it separately as automation.
...
Gnosis stores a typed goal object.

Phase 1 has a simple invariant: one session can have at most one goal.

The goal belongs to the session, not to the whole Gnosis app.
...
Gnosis should enforce the threshold in code, not only in prose.
...
Later, Gnosis can add an internal-context message type if we want Codex-style hidden steering.
...
Attempts are easier to reason about in Gnosis today because `internal/agent` already reports turn completion but does not yet have Codex-style extension token accounting.
...
In Phase 1, that means automatic continuation only starts when Gnosis is idle and the user is not actively interacting with the input.

Gnosis may enqueue a continuation only if:
...
If the user starts typing before the continuation begins, Gnosis should not enqueue the continuation.
...
- Downside: Gnosis already has a simpler model where normal chat waits until the current turn finishes.
