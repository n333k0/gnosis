...
func TestRunDispatchesHelpAndVersionToInjectedOutput(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "version", args: []string{"version"}, want: "gnosis version v1.2.3-test\n"},
	} {
...
func TestRunHeadlessReportsArgumentAndProviderFailures(t *testing.T) {
	t.Run("missing prompt", func(t *testing.T) {
		...
		if !strings.Contains(errOut.String(), "gnosis run: missing prompt") {
	...
	t.Run("provider", func(t *testing.T) {
		...
		t.Chdir(root)
		if err := os.WriteFile(filepath.Join(root, "gnosis.yaml"), []byte("provider: invalid\nmodel: test\n"), 0o644); err != nil {
			t.Fatal(err)
		}
		streams, _, errOut := bufferedStdio()
...
func TestRunRejectsInvalidSessionInput(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "missing resume id", args: []string{"resume"}, code: 2, want: "usage: gnosis resume <session-id>"},
		{name: "malformed resume id", args: []string{"resume", "../bad"}, code: 1, want: "invalid session id"},
		{name: "missing search query", args: []string{"sessions", "search"}, code: 2, want: "usage: gnosis sessions search <query>"},
	} {
		t.Run(tt.name, func(t *testing.T) {
			streams, _, errOut := bufferedStdio()
...
func brokenConfigWorkspace(t *testing.T) string {
	...
	root := t.TempDir()
	t.Setenv("HOME", t.TempDir())
	if err := os.WriteFile(filepath.Join(root, "gnosis.yaml"), []byte("permissions: [invalid\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	return root
}
