...
# Build the gnosis binary (stamps Version via -ldflags)
build:
    go build -ldflags "-X main.Version={{version}}" -o gnosis ./cmd/gnosis

# Run gnosis (dev mode)
dev *args:
    go run -ldflags "-X main.Version={{version}}" ./cmd/gnosis {{ args }}
...
# Install gnosis into a runnable bin directory (stamps Version)
install:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    GOBIN="$bin_dir" go install -ldflags "-X main.Version={{version}}" ./cmd/gnosis
    echo "installed gnosis to $bin_dir/gnosis"
