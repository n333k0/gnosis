...
// config lookups can't pick up the repo's actual gnosis.yaml.
...
func TestLoad_ConfiguredPhasesExtendAndOverrideDefaults(t *testing.T) {
	withTempDir(t, func(dir string) {
		t.Setenv("HOME", dir)
		writeFile(t, filepath.Join(dir, "gnosis.yaml"), `phases:
  review:
...
func TestLoad_RejectsInvalidPhase(t *testing.T) {
	withTempDir(t, func(dir string) {
		...
		for _, want := range []string{"gnosis.yaml", "phases", "help", "reserved"} {
...
func TestLoad_PrefersProjectConfig(t *testing.T) {
	withTempDir(t, func(dir string) {
		t.Setenv("HOME", dir)
		writeFile(t, filepath.Join(dir, "gnosis.yaml"), "model: project-model\n")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("load: %v", err)
		}
		if cfg.Source() != "gnosis.yaml" {
			t.Fatalf("expected source 'gnosis.yaml', got %q", cfg.Source())
		}
...
func TestLoad_RejectsInvalidYAML(t *testing.T) {
	withTempDir(t, func(dir string) {
		t.Setenv("HOME", dir)
		writeFile(t, filepath.Join(dir, "gnosis.yaml"), "model: [unclosed\n")
...
func TestLoad_DefaultsProviderToAnthropic(t *testing.T) {
	withTempDir(t, func(dir string) {
		t.Setenv("HOME", dir)
		writeFile(t, filepath.Join(dir, "gnosis.yaml"), "model: m\n")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("load: %v", err)
		}
...
func TestLoad_OpenAIProviderGetsOpenAIDefaultModel(t *testing.T) {
	withTempDir(t, func(dir string) {
		t.Setenv("HOME", dir)
		writeFile(t, filepath.Join(dir, "gnosis.yaml"), "provider: openai\n")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("load: %v", err)
		}
...
func TestLoad_OpenRouterProviderGetsOpenRouterDefaultModel(t *testing.T) {
	withTempDir(t, func(dir string) {
		t.Setenv("HOME", dir)
		writeFile(t, filepath.Join(dir, "gnosis.yaml"), "provider: openrouter\n")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("load: %v", err)
		}
...
func TestLoad_GoogleProviderGetsGoogleDefaultModel(t *testing.T) {
	withTempDir(t, func(dir string) {
		t.Setenv("HOME", dir)
		writeFile(t, filepath.Join(dir, "gnosis.yaml"), "provider: google\n")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("load: %v", err)
		}
...
func TestLoad_RejectsInvalidOpenAIAuthMode(t *testing.T) {
	withTempDir(t, func(dir string) {
		t.Setenv("HOME", dir)
		writeFile(t, filepath.Join(dir, "gnosis.yaml"), "openai_auth: nope\n")
		_, err := Load()
		if err == nil {
			t.Fatal("expected invalid openai_auth to fail")
		}
		for _, want := range []string{"gnosis.yaml", "openai_auth", OpenAIAuthAPIKey, OpenAIAuthSubscription, "nope"} {
			if !strings.Contains(err.Error(), want) {
				t.Fatalf("error %q does not contain %q", err.Error(), want)
			}
		}
...
func TestLoad_OpenAIAuthDoesNotChangeAnthropicStartupDefault(t *testing.T) {
	withTempDir(t, func(dir string) {
		t.Setenv("HOME", dir)
		writeFile(t, filepath.Join(dir, "gnosis.yaml"), "provider: anthropic\nopenai_auth: subscription\n")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("load: %v", err)
		}
...
func TestLoad_SubagentsProviderGetsProviderDefaultModel(t *testing.T) {
	withTempDir(t, func(dir string) {
		t.Setenv("HOME", dir)
		writeFile(t, filepath.Join(dir, "gnosis.yaml"), "subagents:\n  provider: google\n")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("load: %v", err)
		}
...
func TestLoad_SubagentsBackendAbsentFollowsCoordinator(t *testing.T) {
	withTempDir(t, func(dir string) {
		t.Setenv("HOME", dir)
		writeFile(t, filepath.Join(dir, "gnosis.yaml"), "model: main-model\n")
		cfg, err := Load()
		if err != nil {
			t.Fatalf("load: %v", err)
		}
...
func TestLoad_RejectsUnknownSubagentsProvider(t *testing.T) {
	withTempDir(t, func(dir string) {
		...
		for _, want := range []string{"gnosis.yaml", "subagents.provider", "mystery"} {
			if !strings.Contains(err.Error(), want) {
				t.Fatalf("error %q does not contain %q", err.Error(), want)
			}
		}
