...
Gnosis is a local coding agent distributed as one Go binary. It runs in a
...
specific model vendor. `cmd/gnosis` composes those product capabilities around the
loop through interfaces.

Gnosis is local-first. There is no Gnosis server, database, daemon, plugin runtime, or
...
Gnosis is a Go module licensed under the MIT License.
...
│  gnosis / gnosis chat                gnosis run                   doctor, login, │
...
│ OpenRouter, Google Gemini     │   │ ~/.gnosis sessions and credentials    │
└───────────────────────────────┘   └─────────────────────────────────────┘
```
...
- `cmd/gnosis` owns construction, feature selection, and cross-package wiring.

---

## 2. Process and Startup Lifecycle

Gnosis is a single foreground process. `cmd/gnosis/main.go` keeps process termination
...
~/.gnosis/config.yaml
    ↓ when absent
embedded internal/config/defaults/gnosis.yaml
```
...
`gnosis resume <id>` loads a session, attempts to restore its saved working
...
`gnosis run` uses the same provider interface, agent loop, compactor, base tools,
...
`internal/llm` is the boundary between Gnosis and model vendors.
...
Gnosis maintains both a flattened system prompt and ordered `SystemBlock` values:

```text
Block 1: static Gnosis instructions + skill catalog     cacheable
...
The TUI can add steering while a turn is active. Gnosis waits until the current
...
eligible for concurrent execution. Gnosis:
...
Gnosis treats its VM or sandbox as the security boundary. That environment must
control filesystem, process, network, credential, and external-service access.
Gnosis does not classify shell commands or enforce application permission modes.
...
Gnosis stores user state below `~/.gnosis/`. There is no database.

| Path | Owner | Purpose |
| --- | --- | --- |
| `~/.gnosis/config.yaml` | `internal/config` | optional user configuration |
| `~/.gnosis/auth.json` | `internal/auth` | refreshable OpenAI subscription credentials |
| `~/.gnosis/sessions/index.json` | `internal/session` | session metadata index |
| `~/.gnosis/sessions/<id>.json` | `internal/session` | full transcript, backend metadata, cwd, and usage |
| `~/.gnosis/AGENTS.md` | `internal/projectctx` | optional global project instructions |
| `~/.gnosis/skills/<name>/SKILL.md` | `internal/skills` | user-global reusable skills |
...
one interactive Gnosis process per storage directory. Concurrent processes can
...
```text
~/.gnosis/skills/<name>/SKILL.md
<workspace>/.gnosis/skills/<name>/SKILL.md
```
...
| --- | --- | --- |
| `cmd/gnosis/` | CLI dispatch and application composition | The only layer that wires the full product together. |
...
| `website/` | Astro documentation and marketing site | Not linked into the Gnosis binary or runtime. |

---

## 12. Security Model

Gnosis's main trust boundaries are the model provider, the VM or sandbox that
...
refresh tokens are stored in `~/.gnosis/auth.json`. Debug logging is optional and
...
go build ./cmd/gnosis         local build
just build                 version-stamped ./gnosis binary
...
| # | Limitation | Detail |
| --- | --- | --- |
| 1 | No built-in OS sandbox | Security depends on the VM or sandbox used to run Gnosis. |
...
3. Add provider construction and credential checks in `cmd/gnosis`.
...
should be composed in `cmd/gnosis`, communicate through interfaces or events, and
