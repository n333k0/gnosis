...
Gnosis executes independent, non-mutating tool calls concurrently when a model returns several calls in one response. Mutating and unknown tools remain serial.
...
Gnosis cannot parallelize work that the model emits across separate responses. Add one short stable instruction:
