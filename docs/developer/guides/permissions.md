...
Gnosis trusts its execution environment for security. A VM or sandbox decides
...
## Why Gnosis Does Not Classify Risk
...
Gnosis therefore does not maintain trusted, ask, or read-only permission modes. It
...
Configure a default-empty list in `gnosis.yaml`:
...
Gnosis trims entries when loading configuration, rejects empty entries, and keeps
...
removed. Gnosis rejects them with migration guidance instead of silently changing
their meaning.

Remove the old mode and add `tool_approvals` only for calls you want to confirm.
Run Gnosis inside a suitable VM or sandbox.
