...
Gnosis should make concurrency visible without turning the terminal into a dashboard. Users need to see that several tools or subagents are active, understand what each is doing, and notice partial failure.
...
The implementation adds a small amount of execution identity to events and renders parallel work as an inline group. It uses Gnosis's existing cyan, green, red, white, and muted-grey palette. There are no cards, borders, new panels, or new controls.
...
On narrow terminals, Gnosis removes the activity detail and keyboard hint before truncating the primary state. The status line never wraps.
