...
Gnosis loads the first available config:

1. `./gnosis.yaml`
2. `~/.gnosis/config.yaml`
3. Embedded defaults from `internal/config/defaults/gnosis.yaml`

First hit wins. Config files are not merged.

## Default Config

This is the complete configuration surface. Copy it to `~/.gnosis/config.yaml` or
`./gnosis.yaml` and change only the values you need.

```yaml
# LLM backend: anthropic, openai, openrouter, or google.
provider: anthropic

# OpenAI only: api_key uses OPENAI_API_KEY; subscription uses `gnosis login`.
# openai_auth: api_key
...
[`internal/config/defaults/gnosis.yaml`](https://github.com/n333k0/gnosis/blob/main/internal/config/defaults/gnosis.yaml).
...
| `provider: openai` with `openai_auth: subscription` | ChatGPT/Codex device-code credentials from `~/.gnosis/auth.json` | `internal/llm/openai.CodexClient` |
| `provider: openrouter` | `OPENROUTER_API_KEY` | `internal/llm/openrouter` |
| `provider: google` | `GOOGLE_API_KEY` | `internal/llm/google` |

Subscription credentials are created with `gnosis login` and removed with `gnosis logout`. The docs describe only where credentials live and which flow uses them; token values are never generated into developer docs.
...
Gnosis always provides `/design`, `/plan`, `/build`, and `/review`. The optional
`phases` map adds custom named prompts or overrides default fields by name.
Default phases that are not mentioned remain available even though Gnosis config
...
duplicates keep their first position. Gnosis does not parse shell chains,
...
passed to `gnosis run` or child agents. The VM or sandbox must control filesystem,
process, network, credentials, and external-service access.

The old `permissions:` section has been removed. Gnosis rejects it with migration
