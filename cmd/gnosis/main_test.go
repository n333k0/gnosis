...
import (
	...
	"github.com/n333k0/gnosis/internal/llm/llmtest"
...
func TestPrintVersion(t *testing.T) {
	...
	if got, want := out.String(), "gnosis version v1.2.3-test\n"; got != want {
		t.Fatalf("version output = %q, want %q", got, want)
	}
}

func TestUsageDocumentsVersionAliases(t *testing.T) {
	for _, want := range []string{"gnosis version", "-v", "--version"} {
		if !strings.Contains(usageText, want) {
			t.Fatalf("usage does not contain %q", want)
		}
	}
...
func TestSaveChatSession_NormalizesCodexAdapterProviderForResume(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	authStore := auth.NewStore(filepath.Join(home, ".gnosis", "auth.json"))
