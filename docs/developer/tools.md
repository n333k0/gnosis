...
Gnosis exposes a small built-in tool surface to the model.
...
| `workflow` | Interactive chat | Create or update the visible workflow checklist. Gnosis attaches tool and subagent activity to the active item automatically. |
...
Gnosis relies on its VM or sandbox for security boundaries. Interactive users can
