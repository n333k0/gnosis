...
Gnosis exposes a small built-in tool surface:
...
Headless `gnosis run` receives only the base file, search, and shell tools.
...
The model sees tool specs. Gnosis runs the tool and feeds the result back into the conversation.
...
3. Register it in `cmd/gnosis/main.go`.
...
- Run Gnosis inside a sandbox with the intended filesystem and network boundary.
