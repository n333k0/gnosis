...
Gnosis's coordinator can launch several independent inspection subagents in one response. They run concurrently with tools, return evidence-rich summaries, and cannot race changes in the shared workspace.

The parent remains the coordinator, writer, and final judge.

## Why

Gnosis already had fresh child contexts, bounded supervision, cancellation, retries, event attribution, and a first-class `agent` tool. Before this change, the missing piece was safe concurrent execution.
...
Gnosis runs them concurrently within the existing agent and supervisor limits. Each child gets only its prompt and inspection tools. The parent receives ordered results, compares the evidence, then decides what to change.
...
- Parent cancellation stops cancellation-compliant active children, all event sends unblock, and no Gnosis-owned goroutine leaks.
...
```sh
go test ./internal/factory ./internal/agent ./cmd/gnosis
...
1. Ask Gnosis to inspect three independent code areas with subagents.
