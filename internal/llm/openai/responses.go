...
//
// Gnosis's internal message model is Anthropic-shaped (content blocks of type
...
// apiRequest is the Responses API request body. Store is always false: gnosis is
...
type inputItem struct {
	...
	// conversations can replay items the API requires but gnosis does not interpret.
...
// toInput converts gnosis's content-block messages into Responses input items.
...
func toResponse(out apiResponse) (*llm.Response, error) {
	...
	for _, item := range out.Output {
		...
		// Other item types are ignored: gnosis only understands text and tools, plus
...
// stopReason normalizes the Responses status into gnosis's stop-reason vocabulary
